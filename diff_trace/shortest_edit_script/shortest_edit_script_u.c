...
int main(int argc, char **argv) {
...

  {
    ...
    for (; e; e = e->next) {
      ...
      struct edit * enext = e->next;
      if(e->operation == DELETE && e->next != NULL && enext->operation == INSERT && (e->offset_sequence_one + e->length - 1) == e->next->offset_sequence_one) {
        if(e->length == 1)
          fprintf(stdout, "Changed line %d:\n", e->offset_sequence_one + 1);
        else
          fprintf(stdout, "Changed lines %d-%d:\n", e->offset_sequence_one + 1, e->offset_sequence_one + e->length);
        for(int j = 0; j < e->length; ++j)
          fprintf(stdout, " %s\n", lines1[e->offset_sequence_one + j]);

        fprintf(stdout, "To:\n");

        for(int j = 0; j < enext->length; ++j)
          fprintf(stdout, " %s\n", lines2[enext->offset_sequence_two + j]);
      ...
      case INSERT:
          fprintf(stdout, "inserted after line %d:\n", e->offset_sequence_one + 1);
        for(int j = 0; j < e->length; ++j)
          fprintf(stdout, " %s\n", lines2[e->offset_sequence_two + j]);
        break;
      case DELETE:
        if(e->length == 1)
