...
#include <string>
#include <vector>
#include "../src/srcmlapps.hpp"
#include "../src/Options.hpp"
#include "../src/srcMLTranslator.hpp"
    ...
    for(unsigned int j = i + 1; j < rbuf->buffer->size(); ++j) {
      ...
      if(strcmp((const char *)(*rbuf->buffer)[i]->name, (const char *)(*rbuf->buffer)[j]->name) == 0) {
        ...
        if(edit->operation == DELETE) {
          ...
          // output preeceding text if any
          if(strcmp((const char *)(*rbuf->buffer)[0]->name, "text") == 0)
            outputNode(*(*rbuf->buffer)[0], writer);
