...
bool expr_stmt::is_matchable_impl(const construct& modified) const {
    ...
    const construct::construct_list& original_children = children();
    const construct::construct_list& modified_children = modified_expr_stmt.children();

    if(original_children.empty() && modified_children.empty()) return true;
