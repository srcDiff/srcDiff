...
class change_entity_map {
	...
	public:
        ...
        std::shared_ptr<T> find(srcdiff_type operation, const versioned_string & name) const {

            const_iterator citr = entity.lower_bound(operation);
            const_iterator upper = entity.upper_bound(operation);
            while(citr != upper) {
                if(citr->second->get_name() == name) return citr->second;
                ++citr;
            }
        ...
        summary_output_stream & summarize_pure(summary_output_stream & out, size_t summary_types, srcdiff_type operation) const {
        ...
        summary_output_stream & summarize_modified(summary_output_stream & out, size_t summary_types) const {
