...
void buffer_handler(struct reader_buffer * rbuf, xmlNodePtr node) {
  ...
  if((xmlReaderTypes)node->type == XML_READER_TYPE_END_ELEMENT) {
    ...
    // mark nodes for output
    int unmarked;
    for(unmarked = rbuf->open_diff->back()->open_tags->size() - 1; unmarked > 0
          && (*rbuf->open_diff->back()->open_tags)[unmarked]->marked; --unmarked);

    if(strcmp((const char *)(*rbuf->open_diff->back()->open_tags)[unmarked]->node->name, (const char *)node->name) == 0)
      (*rbuf->open_diff->back()->open_tags)[unmarked]->marked = true;

    // output marked nodes
    while(1) {


      if(!rbuf->open_diff->back()->open_tags->back()->marked)
        return;
      ...
      update_diff_stack(rbuf->open_diff, node, rbuf->stream_source);
      update_diff_stack(rbuf->output_diff, node, rbuf->stream_source);
  ...
  update_diff_stack(rbuf->output_diff, node, operation);
